...
int8_t g_tau07_used_flg=0;
#endif // G23_FPB_128P

#if defined(G23_FPB_128P)
// if TAU0_7 is already used by servo or analogWrite function, this channel cannnot be used for tone output
//
int8_t check_tone_available(uint8_t pin)
{
    if(2 == pin)
    {
        if(g_tau07_used_flg == TAU07_USED_BY_ANALOGWRITE || g_tau07_used_flg == TAU07_USED_BY_SERVO)
        {
            return 0;
        }
        else
        {
            g_tau07_used_flg = TAU07_USED_BY_TONE;
            return 1;
        }
    }
    return 1;
...
void noTone(uint8_t pin)
{
    ...
    if (2 == pin)
    {
        if(g_tau07_used_flg == TAU07_USED_BY_TONE)
        {
            g_tau07_used_flg = 0;
        }
    }
...
#if defined(G23_FPB_128P)
    int8_t temp_tone_available = check_tone_available(pin);
    if(temp_tone_available != 1)
    {
        tone_pin_flg = 0;
    }
#endif // G23_FPB_128P
...
//           R_Config_Through_Start();
        ...
        {
            operating_clock_select = *g_timer_tone_clock_select_reg[tone_channel] & CK02_OPERATION;
        ...
        {
            operating_clock_select = *g_timer_tone_clock_select_reg[tone_channel] & CK03_OPERATION;
        ...
        {
            operating_clock_select = *g_timer_tone_clock_select_reg[tone_channel] & CK00_CK01_OPERATION;
