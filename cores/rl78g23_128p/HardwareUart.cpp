...
void HardwareUart::begin(unsigned long baud, uint16_t config, int rx_buf, int tx_buf)
{
    ...
    switch (_urt_channel)
    {
        ...
        case 1:
            ...
            R_Config_UARTA1_Receive((uint8_t * const)&receive_buffer,1);
#endif /* UART1_CHANNEL==1 */
            break;
        case 2:
#if (UART2_CHANNEL==2)
        ...
        case 4:
#if (UART4_CHANNEL==4)
            phardwareserial[4] = this;
            R_Config_UART3_Create();
            Set_Baudrate(baud);
            Set_SerialPort(SERIAL_TXD4,SERIAL_RXD4);
            Set_Config(config);
...
#if (UART3_CHANNEL == 3 )
            R_Config_UART1_Stop();
            phardwareserial[3] = NULL;
#endif /* (UART2_CHANNEL == 2 ) */
            break;
        case 4:
#if (UART4_CHANNEL == 4 )
            R_Config_UART3_Stop();
            phardwareserial[4] = NULL;
#endif /* (UART2_CHANNEL == 2 ) */
            break;
        case 5:
#if (UART5_CHANNEL == 5 )
            R_Config_UARTA0_Stop();
            phardwareserial[5] = NULL;
#endif /* (UART5_CHANNEL == 5 ) */
            break;
    }
#if defined(FEW_RAM_MODEL) && (FEW_RAM_MODEL == 1)
...
size_t HardwareUart::UART_Send(uint8_t c)
{
    ...
    if(_tx_buffer_tail == _tx_buffer_head)
    {
        ...
        switch (_urt_channel)
        {
            ...
            case 1:
#if ( UART1_CHANNEL == 1 )
                if((ASISA1 & _20_UARTA_DATA_EXIST_IN_TXBA) != 0)
                {
                    return 0;
                }
                err = R_Config_UARTA1_Send((uint8_t * const)&c,1);
#endif /* ( UART1_CHANNEL == 1 ) */
                break;
            case 2:
#if ( UART2_CHANNEL == 2 )
        ...
        if(i == _tx_buffer_tail)
        {
            ...
            else
            {
                ...
                switch (_urt_channel)
                {
                    ...
                    case 1:
#if ( UART1_CHANNEL == 1 )
                        while((ASISA1 & _20_UARTA_DATA_EXIST_IN_TXBA) != 0)
                        {
                            ;
                        }
                        UTIF1 = 0;
...
void HardwareUart::load_char(void){
    ...
    if (i != _tx_buffer_head) {
        ...
        switch (_urt_channel)
        {
            ...
            case 1:
#if (UART1_CHANNEL == 1)
                R_Config_UARTA1_Send((uint8_t * const)&send_buffer,1);
#endif /* (UART1_CHANNEL == 1) */
                break;
            case 2:
#if (UART2_CHANNEL == 2)
...
{
    if (( _urt_channel == 0 ) || ( _urt_channel == 2 )  || ( _urt_channel == 3 ) || (_urt_channel == 4 ))
    {
        ...
        switch (_urt_channel)
        {
            ...
            case 4:
                ...
                SPS1 |= (prs << 4) ;
                SDR12 = ((uint16_t)sdr) << 9 ;
                SDR13 = ((uint16_t)sdr) << 9 ;
#endif /* ( UART2_CHANNEL == 2 ) */
                break;
            default :
                /* Do nothing */
...
void HardwareUart::Set_Config(uint16_t config )
{
    ...
    switch (_urt_channel)
    {
        ...
        {
#if ( UART1_CHANNEL == 1 )
            unsigned short ASIMA11data = _02_UARTA_DIRECTION_LSB | _00_UARTA_DATA_NORMAL | converted_config;
            ASIMA11 = (unsigned char)ASIMA11data;
            break;
        }
#endif /* ( UART1_CHANNEL == 1 ) */
        case 2:  //UART2
        ...
        case 4:  //UART3
        {
            ...
            /* SLCmn1: 0 SLCmn0: 1 when receiving XXXXXXXXXX01XXXX*/
            SCR13data &= ~(_0020_SAU_STOP_2);
            SCR13data |= _0010_SAU_STOP_1;
...
void Set_Char_Serial_to_buf(uint8_t chn)
{
    switch (chn)
    {
        ...
        case 3:
#if (UART3_CHANNEL == 3)
            if(phardwareserial[3] != NULL)
            {
                phardwareserial[3]->store_char();
            }
#endif /* (UART3_CHANNEL == 3) */
            break;
        case 4:
#if (UART4_CHANNEL == 4)
            if(phardwareserial[4] != NULL)
            {
                phardwareserial[4]->store_char();
            }
#endif /* (UART4_CHANNEL == 4) */
            break;
        case 5:
#if (UART5_CHANNEL == 5)
            if(phardwareserial[5] != NULL)
            {
                phardwareserial[5]->store_char();
            }
#endif /* (UART5_CHANNEL == 5) */
            break;
    }
