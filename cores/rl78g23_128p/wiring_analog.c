...
extern __far const PinTableType * __far const pinTablelist[NUM_DIGITAL_PINS];
...
uint8_t g_pwm_create_flg[2]= {0,0};
uint8_t g_analogWrite_enable_interrupt_flag[2]={0,0};
...
extern volatile unsigned short *g_timer_analog_clock_select_reg[PWM_CH_NUM];
extern const uint8_t  pwm_channel_table[PWM_CH_NUM];
extern const uint8_t  pwm_master_channel_table[PWM_CH_NUM];
...
int8_t get_pwm_master_channel(int8_t pwm_num)
{
    if(pwm_num>= 0 && pwm_num<PWM_CH_NUM)
    {
        return (int8_t)pwm_master_channel_table[pwm_num];
    }
    return -1;
}

#if defined(G23_FPB_128P)
// if TAU0_7 is already used by servo or tone function, this channel cannnot be used for PWM output
//
int8_t check_analog_writable(pin_size_t pin)
{
    if(2 == (int8_t)pin)
    {
        if(g_tau07_used_flg == TAU07_USED_BY_TONE || g_tau07_used_flg == TAU07_USED_BY_SERVO)
        {
            return 0;
        }
        else
        {
            g_tau07_used_flg = TAU07_USED_BY_ANALOGWRITE;
            return 1;
        }
    }
    return 1;
...
//        s16Result = _analogRead(g_au8AnalogPinTable[get_analog_channel(analog_pin)]);
        uint8_t u8ADS = g_au8AnalogPinTable[get_analog_channel(analog_pin)];
        s16Result = _analogRead(u8ADS);
    }
    return s16Result;
}
    ...
    {
        {
            ...
            int8_t temp_master_channel = get_pwm_master_channel(pwm_channel);
#if defined(G23_FPB_128P)
            int8_t temp_analog_writable = check_analog_writable(pin);
#endif // G23_FPB_128P
            if(temp_master_channel<0)
            {
                return;
            }
// 20240531
#if defined(G23_FPB_128P)
            else if(temp_analog_writable != 1)
            {
                return;
            }
#endif // G23_FPB_128P
            ...
            {
                pwm_master_channel = (uint8_t)temp_master_channel;
            }
            ...
            {
                if(g_pwm_create_flg[pwm_master_channel] == 0)
                {
                    ...
                    {
                        pwm_frequency = _analogFrequency(pwm_channel_table[pwm_channel],FREQUENCY_MIN_VAL);
//
                ...
                *(g_timer_duty_reg[pwm_channel])   = _analogDuty(value, pwm_ch[pwm_channel].cycle);
                ...
                if(g_u8AnalogWriteAvailableTable[pin] == 0)
                {
                    ...
                    uint8_t master_num = pwm_master_channel_table[pwm_channel];
                    g_analogWrite_enable_interrupt_flag[master_num] = 1;
                ...
                if(g_pwm_create_flg[pwm_master_channel] == 0)
                {
                    ...
                    g_pwm_create_flg[pwm_master_channel] = 1;
...
void analogWriteFrequency(uint32_t Hz) {
    ...
    if((g_pwm_create_flg[0] == 0)&&
        (g_pwm_create_flg[1] == 0))
    {
        pwm_frequency = _analogFrequency(0,Hz);
        for(int i = 0;i<PWM_CH_NUM;i++)
        {
            pwm_ch[i].cycle = pwm_frequency;
        }
...
static uint16_t _analogFrequency (uint8_t pwm_channel, uint32_t u32Hz)
{
    uint16_t Result;
...
//    int8_t pwm_channel = get_pwm_channel(pin);
    ...
    {
        operating_clock_select = *g_timer_analog_clock_select_reg[pwm_channel] & CK02_OPERATION;
    ...
    {
        operating_clock_select = *g_timer_analog_clock_select_reg[pwm_channel] & CK03_OPERATION;
    ...
    {
        operating_clock_select = *g_timer_analog_clock_select_reg[pwm_channel] & CK00_CK01_OPERATION;
