...
#else // ifndef G23_FPB_128P
#include "Config_TAU0_2_Servo.h"
#include "Config_TAU0_6_Servo.h"
#include "Config_TAU0_7_Servo.h"
#endif  // ifndef G23_FPB_128P
#endif  // G22/G23
}
...
#else // ifndef G23_FPB_128P
extern void (*g_fServoInterruptFunc2)(void);
extern void (*g_fServoInterruptFunc6)(void);
extern void (*g_fServoInterruptFunc7)(void);
...
static void Reset_TDR(uint16_t TDRval, timer16_Sequence_t timer)
{
    ...
    {
        if((g_tau07_used_flg != TAU07_USED_BY_ANALOGWRITE) && (g_tau07_used_flg != TAU07_USED_BY_TONE) )
        {
            TDR07 = TDRval;
            TT0 |= _0080_TAU_CH7_STOP_TRG_ON;
            TS0 |= _0080_TAU_CH7_START_TRG_ON;
...
#else // ifndef G23_FPB_128P
static void g_fServoInterruptFunc2_sub(void)
{
    servo_handle_interrupts(_timerTAU02);
}
static void g_fServoInterruptFunc6_sub(void)
{
    servo_handle_interrupts(_timerTAU06);
...
uint8_t Servo::attach(int pin, int min_a, int max_a)
{
    if(this->servoIndex < MAX_SERVOS )
    {
        ...
        if(timer == 0)
        {
            if(isTimerActive(timer) == false)
            {
                g_fServoInterruptFunc2 = g_fServoInterruptFunc2_sub;
                Channel[timer] = -1;
                R_Config_TAU0_2_Servo_Create();
                R_Config_TAU0_2_Servo_Start();
            }
        }
        else if(timer == 1)
        {
        ...
        else
        {
            if((g_tau07_used_flg != TAU07_USED_BY_ANALOGWRITE) && (g_tau07_used_flg != TAU07_USED_BY_TONE) )
            {
                g_tau07_used_flg = TAU07_USED_BY_SERVO;
                if(isTimerActive(timer) == false)
                {
                    g_fServoInterruptFunc7 = g_fServoInterruptFunc7_sub;
                    Channel[timer] = -1;
...
int Servo::readMicroseconds()
{
  ...
  if( this->servoIndex != INVALID_SERVO )
    pulsewidth = (int)ticksToUs(servos[this->servoIndex].ticks)  + TRIM_DURATION ;
  else
    pulsewidth  = 0;
